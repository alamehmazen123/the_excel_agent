    ...
    def run(self, workbook_path: str, options: AnalysisOptions,
        ...
        for i, analyzer in enumerate(analyzers):
            ...
            if use_com and isinstance(analyzer, PivotAnalyzer):
                continue
            # Each analyzer is isolated: one failing must NEVER cost the user every
            # other sheet (the "only Pivot Analysis was generated" symptom). A
            # crash is recorded and surfaced; the remaining sheets still build.
            try:
                if not analyzer.applies_to(profile):
                    continue
                spec = analyzer.run(profile)
            except Exception as exc:                       # noqa: BLE001
                analyzer_errors.append(f"{analyzer.sheet_name}: {exc}")
                continue
            if spec is not None:
                specs.append(spec)
        ...
        try:
            created = write_results(workbook_path, specs, inject=inject,
                                    value_helpers=vhelpers, errors=render_errors)
        except Exception as exc:                           # noqa: BLE001
            # A total write failure must NOT abort the whole run — the COM pivots
            # below can still be built. Record why so it isn't a silent loss.
            analyzer_errors.append(f"analysis sheets could not be written: {exc}")
        analyzer_errors.extend(f"render {e}" for e in render_errors)
        ...
        for err in analyzer_errors:
            result.notes.append(f"Could not build the {err}")
        if semantic.purpose:
            result.notes.append(
