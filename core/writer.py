...
# Numbers are shown with ZERO decimal places everywhere (the values are large
# LBP/USD amounts and counts — decimals only add noise and widen columns).
_NUMFMT = {
...
def _write_heading(ws: Worksheet, spec: SheetSpec, row: int) -> int:
    # Compact heading (was an oversized 20pt / 32px-tall band that dwarfed the
    # tables): a clean 14pt title + a small grey subheading.
    _center_across(ws, row, 1, 8, spec.heading,
                   Font(name="Calibri", size=14, bold=True, color=_BRAND_BLUE),
                   row_height=20)
    row += 1
    if spec.subheading:
        _center_across(ws, row, 1, 8, spec.subheading,
                       Font(size=10, bold=False, color=_GREY))
        row += 1
    return row + 1
...
def _add_chart(ws: Worksheet, chart: ChartSpec, anchor: str,
    ...
    # Value axis shows whole numbers with thousands separators (no decimals).
    try:
        ch.y_axis.numFmt = "#,##0"
    ...
    if chart.kind == ChartKind.BAR:
        # Data labels = Outside End, value shown as a whole number.
...
def _disp_len(value, fmt: Optional[NumberFormat]) -> int:
    """Approximate the DISPLAYED character width of a value once formatted, so
    columns can be sized to actually show the numbers (no '#####')."""
    if value is None:
        return 0
    if isinstance(value, (int, float)) and not isinstance(value, bool):
        n = f"{int(round(value)):,}"
        if fmt == NumberFormat.LBP:
            return len(n) + 4            # "… LBP"
...
def _column_widths(spec: SheetSpec) -> dict[int, float]:
    """Best column widths from the table contents (headers + formatted values),
    clamped so no column is absurdly wide (long decoded names) or too narrow."""
    widths: dict[int, int] = {}
    for t in spec.tables:
        for j, h in enumerate(t.headers, start=1):
            widths[j] = max(widths.get(j, 0), len(str(h)))
        for r in t.rows:
            for j, v in enumerate(r, start=1):
                fmt = t.formats[j - 1] if t.formats and j - 1 < len(t.formats) else None
                widths[j] = max(widths.get(j, 0), _disp_len(v, fmt))
    # Clamp: labels (col 1) up to 42, value columns up to 24; min 9.
    out: dict[int, float] = {}
    for j, w in widths.items():
        hi = 42 if j == 1 else 24
        out[j] = float(max(9, min(hi, w + 2)))
    return out
...
def write_results(source_path: str, specs: list[SheetSpec],
                  ...
                  value_helpers: Optional[tuple] = None,
    ...
    # Render each sheet INDEPENDENTLY: a failure in one (e.g. a chart/data-bar
    # edge case) must not cost the user every other sheet. Failures are recorded
    # in ``errors`` (when provided) so the caller can surface what went wrong.
    created: list[str] = []
    for s in specs:
        if s is None:
            continue
        try:
            created.append(render_sheet(wb, s))
        except Exception as exc:                       # noqa: BLE001
            if errors is not None:
                errors.append(f"{getattr(s, 'name', 'sheet')}: {exc}")
