...
class ExcelFinalizer:
    ...
    def _build_plan(self, excel, wb, plan: list[PivotSpec], source_table,
        ...
        #
        # Some Excel installs (notably LOCALIZED / non-English builds) reject a
        # Table NAME as SourceData with OLE error 0x800a01a8 ("Object required").
        # Retry with the table's fully-qualified RANGE ADDRESS, which every Excel
        # accepts; if even that fails, return gracefully so the caller falls back
        # to static tables instead of losing the whole pivot sheet.
        shared_cache = None
        try:
            shared_cache = wb.PivotCaches().Create(
                SourceType=XL_DATABASE, SourceData=source_table.Name)
        except Exception:
            try:
                rng = source_table.Range
                addr = "'" + source_table.Parent.Name + "'!" + rng.Address
                shared_cache = wb.PivotCaches().Create(
                    SourceType=XL_DATABASE, SourceData=addr)
                self.notes.append("Built pivots from the data range (localized Excel).")
            except Exception as exc:                       # noqa: BLE001
                self.notes.append(f"Could not create the pivot data cache: {exc}")
                return
    ...
    def _autofit_all(self, wb, skip_sheets: set, data_sheet=None) -> None:
        ...
        for ws in wb.Worksheets:
            ...
            try:
                ...
                pass
            # Clamp widths: AutoFit on a long table TITLE or decoded name can blow
            # column A out to a huge width; clamp so columns stay readable and the
            # values still fit (col A up to 44, the rest up to 26, min 9).
            try:
                used = ws.UsedRange
                base, ncols = used.Column, used.Columns.Count
                for i in range(ncols):
                    c = base + i
                    col = ws.Columns(c)
                    hi = 44 if i == 0 else 26
                    w = col.ColumnWidth
                    if w > hi:
                        col.ColumnWidth = hi
                    elif w < 9:
