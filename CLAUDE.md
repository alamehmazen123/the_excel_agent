...
### v1.15.2 — localized-Excel pivots (OLE error 0x800a01a8 on Arabic Excel)
On a LOCALIZED (e.g. Arabic) Excel, `wb.PivotCaches().Create(SourceData=table.Name)`
raised `OLE error 0x800a01a8` ("Object required"), so NO active pivots built and the
sheet fell back to static tables. `_build_plan` now retries the cache with the
table's fully-qualified RANGE ADDRESS (`'Sheet'!$A$1:…`), which every Excel accepts,
and returns gracefully (static fallback) if even that fails — instead of raising out
of the whole pivot build. The English table-name path is unchanged (tried first).
(The grouped "Years" field is referenced by English name and is already wrapped, so
on a localized Excel the pivot still builds; only the Year band may be omitted.)

### v1.15.1 — per-analyzer / per-sheet resilience (fixes "only Pivot Analysis")
A single analyzer or one sheet's render failing used to be able to wipe EVERY
openpyxl sheet, leaving only the COM-built Pivot Analysis on some PCs.
- **`pipeline` wraps each `analyzer.run()`** in try/except — a crash records a
  note (`"Could not build the <Sheet>: <error>"`) and the OTHER sheets still build.
- **`writer.write_results` renders each spec independently** (per-sheet try/except,
  collects into an `errors` list) and the whole `write_results` call is wrapped in
  the pipeline, so even a total write failure still lets the COM pivots build with
  a clear note instead of a silent one-sheet result.
- These notes surface in the completion dialog, so a still-missing sheet now tells
  you WHICH analyzer failed and WHY (diagnostic for frozen-only / data edge cases).
